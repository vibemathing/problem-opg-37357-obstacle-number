"""MIN4 all-simple-path compiler; reused from the attached BIP2 helper.
...
def generate(graph):
    n = graph['n']; need(type(n) is int and 3 <= n <= 8, '3<=n<=8 cap')
    ...
    need(len(targets) <= 12, 'at most twelve targets in this finite interface')
    ...
    clauses = sorted(rows)
    return {'schema':'min4-h2-cnf-v1','n':n,'edges':[list(e) for e in sorted(edges)],
