"""NEXT9 lossless int16-set memory adapter for byte-pinned NB9 consumer.
No clause is removed; producer unchanged; n<=9 bounds every variable index within signed16.
"""
"""NB9 explicit domain/cap extension of supplied NB8 code. See source-reuse.json.
No all-order-type enumeration claim.
"""
"""C31 clause-source audit. Does not import the producer or geometric core."""
from itertools import combinations, product
from struct import pack

def packed(c):
    return pack('>'+'h'*len(c), *c)

def need(ok,why):
    if not ok:raise ValueError(why)

def check(data):
    n=data['n'];need(type(n) is int and 3<=n<=9,'n cap')
    ...
    def norm(c):
        c=set(c)
        return None if any(-v in c for v in c) else packed(sorted(c,key=lambda v:(abs(v),v)))
    ...
    for c,s in zip(data['clauses'],data['origins']):
        ...
        need(packed(c)==norm(row(s)),'row-source mismatch')
        encoded=packed(c);need(encoded in expected,'duplicate or unexpected clause');expected.remove(encoded)
        counts[s[0]]=counts.get(s[0],0)+1
    need(not expected,'missing clauses')
    need(all(data['clauses'][i-1]<data['clauses'][i] for i in range(1,len(data['clauses']))), 'canonical row ordering')
    return {'clauses':len(data['clauses']),'counts':counts,'paths':sum(map(len,Pmap.values())),'max_path_edges':max(len(p)-1 for ps in Pmap.values() for p in ps)}
